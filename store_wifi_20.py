...
from .preproc.fading_model  import add_fading_channel, add_noise
import pdb

parser = argparse.ArgumentParser(description='Trains network on WiFi data')
parser.add_argument('-d', '--exp_dir', type=str, default='/home/rfml/wifi/experiments/NSWC_Crane_Experiments/Test3/converted_3E', help='Base directory for experiment')
...
NSWC_Crane_Experiments/Test3/converted_3E
''' 
parser.add_argument('-n', '--noise', action='store_true', help='Add noise to data (no channel)')
...
parser.add_argument('-snr_tr', '--snr_train', type=np.float, default=500., help='Train SNR in dB (default 500 dB = no noise)')
parser.add_argument('-snr_te', '--snr_test', type=np.float, default=500., help='Test SNR in dB (default 500 dB = no noise)')
...
'''
parser.add_argument('-s', '--seed', type=int, default=0, help='Random seed for channel in training data (default 0)')
...
# dict(signal={}, device_key={}, # Complex signal and device label [0, N-1] from device_map
# 	 sample_rate={}, capture_sample_rate={}, capture_frequency={}, capture_hw={}, 
# 	 freq_lower_edge={}, freq_upper_edge={}, snr={},
...
fc_train =  np.array(train_dict['center_frequency'].values())
fc_test =  np.array(test_dict['center_frequency'].values())
...
# Add only noise
	...
	if args.snr_train < 500:
		train_dict = add_noise(train_dict, snr=args.snr_train, progress=args.progress)
	if args.snr_test < 500:
		test_dict = add_noise(test_dict, snr=args.snr_test, progress=args.progress)
...
if args.channel is True:
	file_name = 'dd-{:}-snr-{:.0f}-{:.0f}-b-{:.0f}-n-{:}-{:}-{:.0f}-pp-{:.0f}-fs-{:.0f}-s-{:}'.format(int(args.diff_day), args.snr_train, args.snr_test, 100*args.beta, args.num_ch_train, args.num_ch_test, args.sample_duration, args.preprocess_type, args.sample_rate, args.seed)
elif args.noise is True:
	file_name = 'snr-{:.0f}-{:.0f}-l-{:.0f}-pp-{:.0f}-fs-{:.0f}'.format(args.snr_train, args.snr_test,args.sample_duration, args.preprocess_type, args.sample_rate)
else:
...
np.savez_compressed(outfile, x_train, y_train, x_test, y_test, fc_train, fc_test)