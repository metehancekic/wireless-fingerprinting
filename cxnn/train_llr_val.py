...
def train(dict_data, aug_test=1, checkpoint_in=None, checkpoint_out=None):
	...
	history = densenet.fit(x=x_train, 
						   ...
						   validation_data=(x_test[:num_test//aug_test], y_test[:num_test//aug_test]), 
						   callbacks=[],
						   steps_per_epoch = num_train//batch_size,
						   validation_steps = 20*num_train//batch_size)
	...
	if aug_test!=0:
		...
		logits_test = model2.predict(x=x_test,
									 batch_size=batch_size,
								 	 verbose=0)		
		logits_test_new = np.zeros((num_test//aug_test, num_classes))
		for i in range(aug_test):
			# list_x_test.append(x_test[i*num_test:(i+1)*num_test])

			logits_test_new += logits_test[i*num_test//aug_test:(i+1)*num_test//aug_test]

		num_test = num_test // aug_test
