	...
	for snr_test in snrs_test:
		...
		for snr_train in snrs_train:
			...
			if snr_train < 500:
				...
				dict_wifi['x_train'] = np.concatenate((x_train_clean.copy(), x_train.copy()), axis=0)
				dict_wifi['y_train'] = np.concatenate((y_train.copy(), y_train.copy()), axis=0)
			...
			data_format_snr = 'snr-aug-{:.0f}-{:.0f}-'.format(snr_train, snr_test) + data_format
