...
def add_custom_fading_channel(frame, 
    ...
    if channel_method == 'FFT':
        ...
        f = np.arange(M+N)[:,None]*sampling_rate/(M+N) # (M+N, 1) -> set of FFT frequencies corresponding to [0, 1, ... M+N-1]
    ...
    if beta <=1:
        if num_ch is None:
            for i in signal_indices:
                ...
                signal_faded = add_fading_channel_to_frame(signal, snr, sampling_rate, seed=None, beta=beta)
            ...
            for i in signal_indices:
                ...
                signal_faded = add_fading_channel_to_frame(signal, snr, sampling_rate, seed=seed, beta=beta)
                signal_faded = normalize(signal_faded)
                data_dict['signal'][i] = signal_faded
        else:
            seed = (seed+10)*100
            for i in signal_indices:
                ...
                signal_faded = add_fading_channel_to_frame(signal, snr, sampling_rate, seed=seed_i, beta=beta)
                signal_faded = normalize(signal_faded)
                data_dict['signal'][i] = signal_faded
    else:
        ...
        if num_ch is None:
            for i in signal_indices:
                ...
                signal_faded = fft_add_fading_channel_to_frame(signal, snr, sampling_rate, seed=None, beta=beta)
                signal_faded = normalize(signal_faded)
                data_dict['signal'][i] = signal_faded
        elif num_ch==1:
            for i in signal_indices:
                ...
                signal_faded = fft_add_fading_channel_to_frame(signal, snr, sampling_rate, seed=seed, beta=beta)
                signal_faded = normalize(signal_faded)
                data_dict['signal'][i] = signal_faded
        else:
            seed = (seed+10)*100
            for i in signal_indices:
                ...
                signal_faded = fft_add_fading_channel_to_frame(signal, snr, sampling_rate, seed=seed_i, beta=beta)
                signal_faded = normalize(signal_faded)
                data_dict['signal'][i] = signal_faded
...
def fft_add_fading_channel_to_frame(frame, snr, sampling_rate, seed=0, beta=None):
    ...
    f = np.arange(M+N)[:,None]*Fs/(M+N) # (M+N, 1) -> set of FFT frequencies corresponding to [0, 1, ... M+N-1]
    tau = np.array(delay_ns)[:,None].transpose()*1e-9 # (1, 7) -> delay_ns converted to s
    H = np.exp(-1j*2*np.pi*(f.dot(tau))) # (M+N, 7) -> FFT of A
    ...
    if beta!=0:
        for i in range(time_idx.size):
            ...
            if t==0:
                ...
                h_rc[i] = np.sin(np.pi*t/Tsym) * np.cos(np.pi*beta*t/Tsym) /((np.pi*t/Tsym)* (1-(2*beta*t/Tsym)**2))    
    else:
...
def root_raised_cosine(shift, N=1600, beta=0.5, Tsym=4e-6, Fs=200e6):
    ...
    for i in range(time_idx.size):
        ...
        if t == 0.0:
            ...
            h_rrc[i] = (beta/np.sqrt(2))*(((1+2/np.pi)* (np.sin(np.pi/(4*beta)))) + ((1-2/np.pi)*(np.cos(np.pi/(4*beta)))))
        elif beta != 0 and t == -Tsym/(4*beta):
            h_rrc[i] = (beta/np.sqrt(2))*(((1+2/np.pi)* (np.sin(np.pi/(4*beta)))) + ((1-2/np.pi)*(np.cos(np.pi/(4*beta)))))
        else:
            h_rrc[i] = (np.sin(np.pi*t*(1-beta)/Tsym) +  4*beta*(t/Tsym)*np.cos(np.pi*t*(1+beta)/Tsym))/ (np.pi*t*(1-(4*beta*t/Tsym)**2)/Tsym)
    return h_rrc
    ...
    with np.errstate(all='raise'):
        try:
            ...
            try:
                ...
                signal_faded += 1.0/np.sqrt(2*signal_faded.size) + 1.0/np.sqrt(2*signal_faded.size)*1j
            except FloatingPointError:
