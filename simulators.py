    ...
    if phy_method_cfo == 1:  # Different offset for each class, same offset for all packets in a class
        ...
        for n in trange(num_classes):
            ...
            for i in ind_n:
                ...
                signal_ch[i:i+1] = add_freq_offset(signal_ch[i:i+1], rand=False,
                                                   df=rv_n.uniform(low=-df_phy_train,
                                                                   high=df_phy_train),
        ...
        for n in trange(num_classes):
            ...
            for i in ind_n:
                ...
                signal_ch[i:i+1] = add_freq_offset(signal_ch[i:i+1], rand=False,
                                                   df=rv_n.uniform(low=-df_phy_test,
                                                                   high=df_phy_test),
        ...
        for n in trange(num_classes):
            ...
            for j in range(phy_method_cfo-1):
                ...
                for i in ind_n[j*num_signals_per_day:(j+1)*num_signals_per_day]:
                    ...
                    signal_ch[i:i+1] = add_freq_offset(signal_ch[i:i+1], rand=False,
                                                       df=rv_n.uniform(
                                                           low=-df_phy_train, high=df_phy_train),
                                                       fc=fc_train_orig[i:i+1],
                                                       fs=sampling_rate)
...
def cfo_compansator(dict_wifi, sampling_rate, data_format):
    ...
    for i in trange(num_train):
        complex_train_removed_cfo[i], freq_train[i] = offset_compensate_preamble(
            complex_train[i], fs=sampling_rate, verbose=False, option=2)
    for i in trange(num_test):
        complex_test_removed_cfo[i], freq_test[i] = offset_compensate_preamble(
            complex_test[i], fs=sampling_rate, verbose=False, option=2)

    dict_wifi['x_train'] = np.concatenate(
        (complex_train_removed_cfo.real[..., None], complex_train_removed_cfo.imag[..., None]), axis=-1)
    dict_wifi['x_test'] = np.concatenate(
        (complex_test_removed_cfo.real[..., None], complex_test_removed_cfo.imag[..., None]), axis=-1)
...
def equalize_channel(dict_wifi, sampling_rate, data_format, verbosity, which_set='x_train'):
    ...
    dict_wifi[which_set] = np.concatenate(
        (complex_train.real[..., None], complex_train.imag[..., None]), axis=2)
