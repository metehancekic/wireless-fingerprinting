'''
Runs a WiFi experiment. Checkpoints and logs are stored in the experiment directory.

Example usage:

python run wifi.py -ch -dd -snr_tr 20 -snr_te 30 -pp 3
...
parser.add_argument('-d', '--exp_dir', type=str, default='/home/rfml/wifi/experiments/exp19', help='Experiment directory')
...
parser.add_argument('-snr_tr', '--snr_train', type=np.float, default=30., help='Train SNR in dB (default 30 dB)')
...
parser.add_argument('-n_te', '--num_ch_test', type=np.int, default=1, help='Number of channels in test data (default 1)')
...
'''
# Optionally save this data to a binary file, and do training later:
np.savez(experiment_directory+'/sym-'+file_name+'.npz', x_train, y_train, x_test, y_test)

# To restore data from a saved npz file:
np_dict = np.load(experiment_directory+'/sym-'+file_name+'.npz')
dict_wifi = {}
...
# Checkpoint filename
checkpoint = experiment_directory + '/ckpt-' + file_name + '.h5'

# Train a network on this data
train_output, model_name, summary = train(dict_wifi, checkpoint_in=None, checkpoint_out=checkpoint)

# Write logs (accuracies, network parameters)
with open(experiment_directory + '/logs-' + + file_name + '.txt', 'a+') as f:
	f.write('\n\n-----------------------\n'+str(model_name)+'\n\n')
